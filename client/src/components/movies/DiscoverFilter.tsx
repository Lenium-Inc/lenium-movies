...
export function DiscoverFilter({
  ...
  if (!isOpen) {
    return (
      <button
        onClick={onToggle}
        className="flex items-center gap-2 rounded-md border border-white/10 bg-white/[0.03] px-4 py-2.5 text-xs font-semibold text-[#c4c4c0] transition hover:border-white/25 hover:bg-white/5 hover:text-white"
        aria-expanded="false"
  ...
  return (
    <div id="discover-filters" className="animate-in slide-in-from-top-2 duration-200">
      ...
      {/* Results summary bar */}
      <div className="mt-3 flex items-center justify-between text-xs text-[#89898e]">
        <span>
          {isLoading ? (
            <>
