...
export default function Home() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#0b0b0e] text-[#f1f1ee] selection:bg-[#d7d7d3] selection:text-[#0b0b0e]">
      ...
      <main>
        <section className="relative isolate flex min-h-[650px] items-end overflow-hidden pb-16 pt-32 md:min-h-[700px] md:pb-20">
          ...
          <div className="mx-auto grid w-full max-w-[1440px] grid-cols-1 px-5 md:px-10 lg:grid-cols-[minmax(0,520px)_1fr]">
            <div className="animate-rise">
              ...
              <div className="mt-9 flex flex-wrap gap-3"><button onClick={() => setSelectedMovie(movies[0])} className="group flex items-center gap-2 rounded-full bg-[#d7d7d3] px-5 py-3 text-xs font-bold text-[#0d0d0f] transition hover:bg-[#ffffff]"><Play className="h-3.5 w-3.5 fill-current transition group-hover:scale-110" /> Play trailer</button><button onClick={() => toggleSaved(movies[0])} className="flex items-center gap-2 rounded-full border border-white/20 bg-white/[0.06] px-5 py-3 text-xs font-semibold text-[#f3eee7] backdrop-blur-sm transition hover:bg-white/15"><Bookmark className="h-3.5 w-3.5" /> {savedIds.includes(1) ? "Saved" : "Save"}</button></div>
            </div>
        ...
        <div className="mx-auto max-w-[1440px] px-5 pb-20 md:px-10">
          ...
          {activeView === "My list" ? <section className="pt-5"><SectionHeading eyebrow="Your shelf" title="Saved for later" action="" />{filteredMovies.length ? <div className="grid grid-cols-2 gap-x-4 gap-y-9 sm:grid-cols-4 lg:grid-cols-6">{filteredMovies.map((movie) => <MovieCard key={movie.id} movie={movie} onSelect={() => setSelectedMovie(movie)} isSaved={savedIds.includes(movie.id)} onToggleSave={() => toggleSaved(movie)} />)}</div> : <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.025] px-6 py-20 text-center"><Bookmark className="mx-auto mb-3 h-6 w-6 text-[#d7d7d3]" /><h3 className="font-display text-2xl">Your list is waiting</h3><p className="mx-auto mt-2 max-w-sm text-sm text-[#8f8b86]">Save a few films while you explore and they’ll show up here.</p></div>}</section> : <>
            ...
            <section className="mt-16"><SectionHeading eyebrow="Pick up where you left off" title="Continue watching" action="See history" /><div className="grid gap-5 md:grid-cols-2"><button onClick={() => setSelectedMovie(movies[2])} className="group relative flex min-h-[150px] overflow-hidden rounded-xl border border-white/[0.08] bg-[#151519] text-left"><img loading="lazy" decoding="async" src={movies[2].backdrop} alt="" className="absolute inset-0 h-full w-full object-cover opacity-40 transition duration-500 group-hover:scale-105 group-hover:opacity-55" /><div className="absolute inset-0 bg-gradient-to-r from-[#101014] via-[#101014]/80 to-transparent" /><div className="relative flex w-full items-center gap-5 p-5"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[#d7d7d3] text-[#0b0b0e]"><Play className="ml-0.5 h-4 w-4 fill-current" /></div><div><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#d7d7d3]">Continue watching</p><h3 className="font-display text-2xl">Soft Focus</h3><p className="mt-1 text-xs text-[#aaa5a0]">32 min left <span className="px-1.5 text-[#5f5c5a]">•</span> Episode 01</p><div className="mt-4 h-1 w-44 overflow-hidden rounded-full bg-white/15"><div className="h-full w-[62%] rounded-full bg-[#d7d7d3]" /></div></div></div></button><div className="flex items-center justify-between rounded-xl border border-white/[0.08] bg-[#151519] px-5 py-4"><div><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#b8b8b5]">Tonight's edit</p><h3 className="font-display text-2xl">Slow cinema, warm light</h3><p className="mt-1 max-w-[290px] text-xs leading-5 text-[#8d8984]">Four patient films for when the world feels a little too loud.</p><button onClick={() => { setActiveCategory("Drama"); setActiveView("Explore"); }} className="mt-4 flex items-center gap-2 text-xs font-bold text-[#e4e4e0] transition hover:text-white">Explore the edit <ArrowRight className="h-3.5 w-3.5" /></button></div><div className="hidden h-24 w-20 rotate-6 overflow-hidden rounded-md bg-[#29292d] sm:block"><img loading="lazy" decoding="async" src={movies[5].poster} alt="" className="h-full w-full object-cover opacity-80" /></div></div></div></section>
      ...
      {selectedMovie && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" onClick={() => setSelectedMovie(null)}><div onClick={(event) => event.stopPropagation()} className="relative max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-t-2xl border border-white/10 bg-[#151519] shadow-2xl sm:rounded-2xl"><div className="relative h-56 overflow-hidden sm:h-72"><img loading="lazy" decoding="async" src={selectedMovie.backdrop} alt="" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#151519] via-transparent to-black/10" /><button onClick={() => setSelectedMovie(null)} aria-label="Close details" className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-black/35 text-white backdrop-blur-md hover:bg-black/60"><X className="h-4 w-4" /></button><div className="absolute bottom-5 left-6 right-6"><span className="rounded-full bg-[#d7d7d3] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-[#0b0b0e]">{selectedMovie.mood}</span><h2 className="mt-3 font-display text-4xl font-semibold tracking-tight sm:text-5xl">{selectedMovie.title}</h2></div></div><div className="grid gap-8 p-6 sm:grid-cols-[1fr_180px] sm:p-8"><div><div className="flex flex-wrap items-center gap-3 text-xs text-[#a7a29c]"><span className="flex items-center gap-1 text-[#d7d7d3]"><Star className="h-3.5 w-3.5 fill-current" /> {selectedMovie.score}</span><span>{selectedMovie.year}</span><span>{selectedMovie.runtime}</span><span className="rounded border border-white/20 px-1.5 py-0.5">{selectedMovie.rating}</span>{selectedMovie.genre.map((genre) => <span key={genre} className="text-[#d2ccc3]">{genre}</span>)}</div><p className="mt-5 max-w-xl text-sm leading-6 text-[#c3beb7]">{selectedMovie.synopsis}</p><p className="mt-4 text-xs text-[#8b8781]">Directed by <span className="text-[#ddd7ce]">{selectedMovie.director}</span></p><div className="mt-7 flex flex-wrap gap-3"><button onClick={() => toast.success("Playback is ready for authorized titles", { duration: 2200 })} className="flex items-center gap-2 rounded-full bg-[#d7d7d3] px-5 py-3 text-xs font-bold text-[#0b0b0e] hover:bg-[#ffffff]"><Play className="h-3.5 w-3.5 fill-current" /> Start watching</button><button onClick={() => toggleSaved(selectedMovie)} className="flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.04] px-5 py-3 text-xs font-semibold text-[#f1ece4] hover:bg-white/10">{savedIds.includes(selectedMovie.id) ? <Check className="h-3.5 w-3.5" /> : <Bookmark className="h-3.5 w-3.5" />} {savedIds.includes(selectedMovie.id) ? "In your list" : "Add to list"}</button></div></div><div className="hidden space-y-3 sm:block"><div className="rounded-xl border border-white/10 bg-white/[0.035] p-4"><div className="mb-3 flex items-center gap-2 text-xs font-semibold text-[#e5dfd6]"><Info className="h-3.5 w-3.5 text-[#d7d7d3]" /> Details</div><dl className="space-y-2 text-[11px] text-[#8f8b86]"><div className="flex justify-between gap-4"><dt>Audio</dt><dd className="text-right text-[#cbc5bd]">English 5.1</dd></div><div className="flex justify-between gap-4"><dt>Subtitles</dt><dd className="text-right text-[#cbc5bd]">EN, ES, FR</dd></div><div className="flex justify-between gap-4"><dt>Availability</dt><dd className="text-right text-[#c5c5c1]">Streaming now</dd></div></dl></div></div></div></div></div>}
    </div>
  );
}
