...
function SectionHeading({ eyebrow, title, action = "View all" }: { eyebrow?: string; title: string; action?: string }) {
  return (
    <div className="mb-5 flex items-end justify-between gap-4">
      <div>
        {eyebrow && <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.24em] text-[#d7d7d3]">{eyebrow}</p>}
        <h2 className="font-display text-2xl font-semibold tracking-tight text-[#f1f1ee] md:text-3xl">{title}</h2>
      </div>
      <button className="group hidden items-center gap-2 pb-1 text-xs font-semibold text-[#a9a59f] transition hover:text-[#d7d7d3] sm:flex">
  ...
  return (
    <article className="group min-w-[144px] flex-1 sm:min-w-[170px]">
      <div className="relative aspect-[2/3] overflow-hidden rounded-[6px] bg-[#1a1a1e] shadow-[0_10px_35px_rgba(0,0,0,0.22)]">
        <img loading="lazy" decoding="async" src={movie.poster} alt={movie.title} className="h-full w-full object-cover transition duration-500 ease-out group-hover:scale-[1.05]" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/0 to-black/10 opacity-70" />
        <button aria-label={`Play ${movie.title}`} onClick={onSelect} className="absolute left-1/2 top-1/2 grid h-11 w-11 -translate-x-1/2 -translate-y-1/2 scale-90 place-items-center rounded-full bg-[#f0f0ed] text-[#101014] opacity-0 shadow-[0_0_0_8px_rgba(247,239,228,0.12)] transition duration-200 group-hover:scale-100 group-hover:opacity-100">
          <Play className="ml-0.5 h-4 w-4 fill-current" />
        </button>
        <button aria-label={isSaved ? `Remove ${movie.title} from list` : `Save ${movie.title}`} onClick={(event) => { event.stopPropagation(); onToggleSave(); }} className={`absolute right-2.5 top-2.5 grid h-8 w-8 place-items-center rounded-full backdrop-blur-md transition ${isSaved ? "bg-[#d7d7d3] text-[#0b0b0e]" : "bg-black/35 text-white/85 hover:bg-white/20"}`}>
        ...
        <div className="absolute bottom-3 left-3 flex items-center gap-1 text-[10px] font-semibold tracking-wide text-white/85">
          <Star className="h-3 w-3 fill-[#d7d7d3] text-[#d7d7d3]" /> {movie.score}
        </div>
      </div>
      <button onClick={onSelect} className="mt-3 block text-left">
        <h3 className="line-clamp-1 text-sm font-semibold text-[#eee9df] transition group-hover:text-[#d7d7d3]">{movie.title}</h3>
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#0b0b0e] text-[#f1f1ee] selection:bg-[#d7d7d3] selection:text-[#0b0b0e]">
      <header className="absolute inset-x-0 top-0 z-40 border-b border-white/[0.07] bg-gradient-to-b from-black/75 to-transparent">
        <div className="mx-auto flex h-[76px] max-w-[1440px] items-center gap-5 px-5 md:px-10">
          <button onClick={() => handleNav("Explore")} className="flex items-center gap-2.5" aria-label="Lenflix home">
            <span className="grid h-8 w-8 place-items-center rounded-full bg-[#d7d7d3] text-[#0b0b0e] shadow-[0_0_24px_rgba(244,176,110,0.28)]"><CirclePlay className="h-4 w-4 fill-current" /></span>
            <span className="font-display text-[17px] font-semibold tracking-[-0.03em]">Lenflix<span className="text-[#d7d7d3]">.</span></span>
          </button>
          <nav className="ml-7 hidden items-center gap-7 text-[12px] font-semibold text-[#b6b1ac] md:flex">
            {['Explore', 'New & notable', 'Collections', 'My list'].map((item) => <button key={item} onClick={() => handleNav(item === "New & notable" ? "New" : item)} className={`relative py-2 transition hover:text-white ${activeView === (item === "New & notable" ? "New" : item) ? "text-white" : ""}`}>{item}{activeView === (item === "New & notable" ? "New" : item) && <span className="absolute -bottom-1 left-0 h-px w-full bg-[#d7d7d3]" />}</button>)}
          </nav>
          <div className="ml-auto flex items-center gap-2.5">
            <div className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/[0.055] px-3 py-2 text-[#aaa6a1] transition focus-within:border-[#d7d7d3]/60 focus-within:bg-white/[0.08] sm:flex">
            ...
            {isAuthenticated ? <button onClick={() => logout()} className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/[0.07] px-3 py-2 text-xs font-semibold text-[#d9d3cb] transition hover:bg-white/15 sm:flex"><UserRound className="h-3.5 w-3.5" /> {user?.name?.split(" ")[0] ?? "Account"}</button> : <button onClick={() => startLogin()} className="hidden rounded-full bg-[#d7d7d3] px-4 py-2 text-xs font-bold text-[#0b0b0e] transition hover:bg-[#ffffff] sm:block">Sign in</button>}
          </div>
        </div>
      ...
      <main>
        <div className="relative z-30 border-b border-white/10 bg-[#d7d7d3] px-5 py-2 text-center text-[10px] font-bold uppercase tracking-[0.16em] text-[#0b0b0e]">Preview only · Fixture catalogue · No real ratings, rights, or playback</div>
        <section className="relative isolate flex min-h-[650px] items-end overflow-hidden pb-16 pt-32 md:min-h-[700px] md:pb-20">
          <div className="absolute inset-0 -z-20 bg-[#101012]" />
          <img loading="eager" fetchPriority="high" decoding="async" src={movies[0].backdrop} alt="" className="absolute inset-0 -z-10 h-full w-full object-cover object-center opacity-70" />
          ...
          <div className="mx-auto grid w-full max-w-[1440px] grid-cols-1 px-5 md:px-10 lg:grid-cols-[minmax(0,520px)_1fr]">
            <div className="animate-rise">
              <div className="mb-6 flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.23em] text-[#d7d7d3]"><span className="h-px w-8 bg-[#d7d7d3]" /> Lenflix original <span className="text-[#8c8884]">/</span> 01</div>
              <h1 className="max-w-[530px] font-display text-6xl font-semibold leading-[0.92] tracking-[-0.055em] text-[#f5f5f2] sm:text-7xl md:text-[94px]">After<br /><span className="text-[#d7d7d3]">light</span></h1>
        ...
        <div className="mx-auto max-w-[1440px] px-5 pb-20 md:px-10">
          ...
          {activeView === "My list" ? <section className="pt-5"><SectionHeading eyebrow="Your shelf" title="Saved for later" action="" />{filteredMovies.length ? <div className="grid grid-cols-2 gap-x-4 gap-y-9 sm:grid-cols-4 lg:grid-cols-6">{filteredMovies.map((movie) => <MovieCard key={movie.id} movie={movie} onSelect={() => setSelectedMovie(movie)} isSaved={savedIds.includes(movie.id)} onToggleSave={() => toggleSaved(movie)} />)}</div> : <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.025] px-6 py-20 text-center"><Bookmark className="mx-auto mb-3 h-6 w-6 text-[#d7d7d3]" /><h3 className="font-display text-2xl">Your list is waiting</h3><p className="mx-auto mt-2 max-w-sm text-sm text-[#8f8b86]">Save a few films while you explore and they’ll show up here.</p></div>}</section> : <>
            ...
            <section className="mt-16"><SectionHeading eyebrow="Playback memory" title="Continue watching" action="Unavailable" /><div className="grid gap-5 md:grid-cols-2"><div className="rounded-xl border border-dashed border-white/[0.12] bg-[#151519] p-6"><p className="text-sm font-semibold text-[#f1f1ee]">History and resume progress are not connected yet.</p><p className="mt-2 max-w-md text-xs leading-5 text-[#8f8b86]">This preview does not store watch history or playback position. Those controls will appear after authorized playback and persistent account APIs are configured.</p></div><div className="flex items-center justify-between rounded-xl border border-white/[0.08] bg-[#151519] px-5 py-4"><div><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#b8b8b5]">Tonight's edit</p><h3 className="font-display text-2xl">Slow cinema, warm light</h3><p className="mt-1 max-w-[290px] text-xs leading-5 text-[#8d8984]">Four patient films for when the world feels a little too loud.</p><button onClick={() => { setActiveCategory("Drama"); setActiveView("Explore"); }} className="mt-4 flex items-center gap-2 text-xs font-bold text-[#e4e4e0] transition hover:text-white">Explore the edit <ArrowRight className="h-3.5 w-3.5" /></button></div><div className="hidden h-24 w-20 rotate-6 overflow-hidden rounded-md bg-[#29292d] sm:block"><img loading="lazy" decoding="async" src={movies[5].poster} alt="" className="h-full w-full object-cover opacity-80" /></div></div></div></section>

            <section className="mt-16"><SectionHeading eyebrow="More to explore" title="More to get lost in" /><div className="grid grid-cols-2 gap-x-4 gap-y-9 sm:grid-cols-4 lg:grid-cols-6">{movies.slice(2, 8).map((movie) => <MovieCard key={movie.id} movie={movie} onSelect={() => setSelectedMovie(movie)} isSaved={savedIds.includes(movie.id)} onToggleSave={() => toggleSaved(movie)} />)}</div></section>
          </>}
        </div>
      </main>

      <footer className="border-t border-white/[0.07] px-5 py-10 md:px-10"><div className="mx-auto flex max-w-[1440px] flex-col justify-between gap-6 text-xs text-[#77747a] sm:flex-row sm:items-center"><div className="flex items-center gap-2.5 text-[#d6d0c7]"><span className="grid h-6 w-6 place-items-center rounded-full bg-[#d7d7d3] text-[#0b0b0e]"><CirclePlay className="h-3 w-3 fill-current" /></span><span className="font-display text-sm">Lenflix<span className="text-[#d7d7d3]">.</span></span></div><div className="flex flex-wrap gap-5"><span>About</span><span>For filmmakers</span><span>Rights & availability</span><span>Privacy</span></div><span>© 2024 Lenflix</span></div></footer>

      {selectedMovie && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" onClick={() => setSelectedMovie(null)}><div onClick={(event) => event.stopPropagation()} className="relative max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-t-2xl border border-white/10 bg-[#151519] shadow-2xl sm:rounded-2xl"><div className="relative h-56 overflow-hidden sm:h-72"><img loading="lazy" decoding="async" src={selectedMovie.backdrop} alt="" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#151519] via-transparent to-black/10" /><button onClick={() => setSelectedMovie(null)} aria-label="Close details" className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-black/35 text-white backdrop-blur-md hover:bg-black/60"><X className="h-4 w-4" /></button><div className="absolute bottom-5 left-6 right-6"><span className="rounded-full bg-[#d7d7d3] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-[#0b0b0e]">Preview fixture</span><h2 className="mt-3 font-display text-4xl font-semibold tracking-tight sm:text-5xl">{selectedMovie.title}</h2></div></div><div className="grid gap-8 p-6 sm:grid-cols-[1fr_180px] sm:p-8"><div><div className="flex flex-wrap items-center gap-3 text-xs text-[#a7a29c]"><span className="flex items-center gap-1 text-[#d7d7d3]"><Star className="h-3.5 w-3.5 fill-current" /> Preview score {selectedMovie.score}</span><span>{selectedMovie.year}</span><span>{selectedMovie.runtime}</span><span className="rounded border border-white/20 px-1.5 py-0.5">Preview rating {selectedMovie.rating}</span>{selectedMovie.genre.map((genre) => <span key={genre} className="text-[#d2ccc3]">{genre}</span>)}</div><p className="mt-5 max-w-xl text-sm leading-6 text-[#c3beb7]">{selectedMovie.synopsis}</p><p className="mt-4 text-xs text-[#8b8781]">Preview director: <span className="text-[#ddd7ce]">{selectedMovie.director}</span></p><div className="mt-7 flex flex-wrap gap-3"><button onClick={() => toast.info(getPlaybackUnavailableState().message, { duration: 3000 })} className="flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.04] px-5 py-3 text-xs font-semibold text-[#f1ece4] hover:bg-white/10"><Play className="h-3.5 w-3.5" /> Playback unavailable</button><button onClick={() => toggleSaved(selectedMovie)} className="flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.04] px-5 py-3 text-xs font-semibold text-[#f1ece4] hover:bg-white/10">{savedIds.includes(selectedMovie.id) ? <Check className="h-3.5 w-3.5" /> : <Bookmark className="h-3.5 w-3.5" />} {savedIds.includes(selectedMovie.id) ? "Saved in preview" : "Save in preview"}</button></div></div><div className="hidden space-y-3 sm:block"><div className="rounded-xl border border-white/10 bg-white/[0.035] p-4"><div className="mb-3 flex items-center gap-2 text-xs font-semibold text-[#e5dfd6]"><Info className="h-3.5 w-3.5 text-[#d7d7d3]" /> Details</div><dl className="space-y-2 text-[11px] text-[#8f8b86]"><div className="flex justify-between gap-4"><dt>Audio</dt><dd className="text-right text-[#cbc5bd]">Not configured</dd></div><div className="flex justify-between gap-4"><dt>Subtitles</dt><dd className="text-right text-[#cbc5bd]">Not configured</dd></div><div className="flex justify-between gap-4"><dt>Availability</dt><dd className="text-right text-[#c5c5c1]">Not available</dd></div></dl></div></div></div></div></div>}
    </div>
  );
}
